...
def main(config_path):
    ...
    criteria = [ FocalLoss(gamma=2) for _ in range(4) ]
    ...
    if checkpoint_path.exists():
        ...
        print(f"Resumed from epoch {start_epoch}. Best F1 so far: {best_metric:.4f}")
    else:
    ...
    for epoch in range(start_epoch, config['training']['epochs']):
        ...
        if 'Validation' in dataloaders:
            ...
            avg_f1_score = np.mean([
                val_metrics['f1_macro_boredom'],
                val_metrics['f1_macro_engagement'],
                val_metrics['f1_macro_confusion'],
                val_metrics['f1_macro_frustration']
            ])
            current_metric = avg_f1_score
            print(f"Validation Exact Match Accuracy: {val_metrics['exact_match_accuracy']:.4f}")
            print(f"Validation Average F1-Score: {current_metric:.4f}")
        ...
        if current_metric > best_metric:
            ...
            epochs_no_improve = 0
            print(f"New best model saved with Average F1-Score: {best_metric:.4f}")
            torch.save({
                'epoch': epoch + 1, 'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict(),
                'best_metric': best_metric, 'epochs_no_improve': epochs_no_improve
            ...
            epochs_no_improve += 1
            print(f"No improvement in validation F1-Score for {epochs_no_improve} epoch(s).")
    ...
    if (output_dir / 'best_model.pth').exists():
        best_model_checkpoint = torch.load(output_dir / 'best_model.pth', weights_only=False)
