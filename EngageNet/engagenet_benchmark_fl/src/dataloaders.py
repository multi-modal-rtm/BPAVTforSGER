...
def get_transforms(config, split='train'):
    ...
    # --- ADDED: More aggressive data augmentation for the training set ---
    if split == 'train':
    ...
    # Normalization is always the last step
...
#             raise FileNotFoundError(f"Could not load annotations from {annotation_file}")
#         self.config = config
#         self.split = split # e.g., 'Train', 'Validation', 'Test'
#         self.transform = transform
#         self.audio_transform = audio_transform
#         self.modality = config.modality
...
#         # --- BUG FIX: Construct the correct path using the split name ---
#         split_name_capitalized = self.split.capitalize() # Ensures correct folder name (e.g., 'Train', 'Validation')
#         video_path = os.path.join(self.config.data_root, split_name_capitalized, f"{video_id}.mp4")
...
#     ]
#     if split == 'train':
#         video_transform_list.insert(1, transforms.RandomHorizontalFlip())
